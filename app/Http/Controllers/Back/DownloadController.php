...
class DownloadController extends Controller
{
    ...
    {   
      ...
      $product_list = Post::where('parent_menu_id','=',@$facility_menu_id->id)->orWhere('parent_menu_id','=',@$nri_menu_id->id)->orWhere('parent_menu_id','=',@$mha_menu_id->id)->whereNotNull('sub_menu_id')->orderBy('sub_menu_id', 'asc')->whereNull('deleted_at')->pluck('title','id')->prepend('--Select--','');
    ...
    // dd($request->all());
      ...
      {      
       ...
       $product_list = Post::where('parent_menu_id','=',@$facility_menu_id->id)->orWhere('parent_menu_id','=',@$nri_menu_id->id)->orWhere('parent_menu_id','=',@$mha_menu_id->id)->whereNotNull('sub_menu_id')->orderBy('parent_menu_id', 'asc')->whereNull('deleted_at')->pluck('title','id')->prepend('--Select--','');
 ...
 public function getExportGraduationReport(Request $request)
 {
      ...
      $dataHeader = array('SL No.','Name','Phone No.','Email','Company', 'No. of Vehicles');
      ...
      foreach ($student_details as $report_key => $report_value) {
        $quote_details[$i][] = $report_key+1;
        $quote_details[$i][] = $report_value->name;
        $quote_details[$i][] = $report_value->phone;
        $quote_details[$i][] = $report_value->email;
        $quote_details[$i][] = $report_value->company;
        $quote_details[$i][] = $report_value->no_of_vehicle;
        $i++;
      }
      //dd($quote_details, $dataHeader);
      $export = new QuoteExport($quote_details, $dataHeader);  
