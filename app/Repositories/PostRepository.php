...
class PostRepository
{
    ...
    public function store($request)
    {
        ...
        $news_id = ParentMenu::select('id')->where('layout_name', 'BLOG')->first();
        // $case_study_id = SubMenu::select('id')->where('slug', '=', 'case-studies')->first();
        $service_id = ParentMenu::select('id')->where('layout_name', 'SERVICES')->first();

        $menu_id = (int)$request->parent_menu_id;
        $sub_menu_id = (int)$request->sub_menu_id;

        if ($news_id->id != $menu_id && $service_id->id != $menu_id) {
